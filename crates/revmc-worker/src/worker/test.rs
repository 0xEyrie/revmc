...
use revm_primitives::{
    hex::FromHex, AccessList, AccessListItem, BlockEnv, Bytecode, Bytes, TxEnv, TxKind, B256,
};
...
fn test_compiler_cache_load_access_list() {
    ...
    evm.context.evm.inner.env.tx.access_list = access_list.0.clone();

    if let Err(err) =
        ext_worker.cache_load_access_list(access_list.to_vec(), CacheDB::new(EmptyDB::new()))
    {
        println!("While cache_load_access_list: {:#?}", err);
    };

    if let Err(err) = evm.transact() {
        println!("Whilte evm transact: {:#?}", err)
    };
}
